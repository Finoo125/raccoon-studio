...
VENDORED verbatim from TenStrip/10S-Comfy-nodes @ c231acaa (v1.9.7, MIT -- see
LICENSE-10S_Nodes in this folder). Registered as `RaccoonLTXFaceIdentity` so
nothing collides if the pack is ever installed alongside; the class name is left
as-is to keep diffs against upstream readable.

Two edits, both mechanical:
  - `_detect_face_bbox` comes from our `face_detect` (three functions lifted out
    of upstream's `latent_likeness_guide`) instead of that whole module, which
    would drag in `latent_tiled_sampler` for helpers we never call.
  - the registration block below.

REPLACES reference conditioning, it does not stack with it: both inject
reference tokens, and TenStrip's own FaceID workflow and PlagueKind's V7 each
drop `LTXReferenceConditioning` entirely when this node is in the graph.
`RaccoonLTXReferenceEnable` still belongs in the graph -- this node calls
`apply_global_patches()` itself and that is idempotent.

Pinned deliberately: upstream reworked this node three times in July 2026
(v1.9.4, v1.9.5, v1.9.7) and the author calls it "still exploring". Needs
ltx_reference_enable at >= v1.9.6 for the RoPE phase-rotation path.
...
from .face_detect import _detect_face_bbox
...
NODE_CLASS_MAPPINGS = {"RaccoonLTXFaceIdentity": LTXFaceIdentityReinforcer}
NODE_DISPLAY_NAME_MAPPINGS = {"RaccoonLTXFaceIdentity": "Raccoon LTX Face Identity"}
