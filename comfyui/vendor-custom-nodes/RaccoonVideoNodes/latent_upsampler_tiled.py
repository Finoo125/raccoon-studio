...
VENDORED
================================================================================
Vendored from TenStrip/10S-Comfy-nodes @ c412da52 (MIT — see LICENSE-10S_Nodes
in this folder), file `latent_upsampler_tiled.py`. Registered as
`RaccoonLatentUpsamplerTiled`, not under the upstream name, so nothing collides
if the pack is ever installed alongside. The class name is left verbatim to keep
future diffs against upstream readable.

Why vendored: this node is the last thing pinning ComfyUI core. It treats the
output of core's `LatentUpscaleModelLoader` as a bare `nn.Module`, and newer
ComfyUI wraps that in a `CoreModelPatcher` — so an I2V job samples all its steps
and then dies at the upscale stage with `AttributeError`. Vendoring it lets us
carry the one-line unwrap (see `upsample_latent_tiled`) instead of freezing the
whole engine on a 2026-07-18 revision to keep one node happy.

Upstream is otherwise unmodified.
...
class LTXVLatentUpsamplerTiled:
    ...
    def upsample_latent_tiled(self, samples, upscale_model, vae,
                              ...
                              debug=False):
        # ComfyUI used to return a bare nn.Module from LatentUpscaleModelLoader
        # and now returns a CoreModelPatcher wrapping one. Everything below wants
        # the module itself -- .parameters() for the dtype, module_size(),
        # .to()/.cpu(), and calling it -- so unwrap once, here, and accept both
        # shapes. Without this the job samples every step and only then dies, at
        # the upscale stage, with AttributeError.
        #
        # ponytail: keeps this node's own .to()/.cpu() handling rather than
        # switching to model_management.load_model_gpu(patcher). Manual movement
        # still works and is a far smaller change, but it bypasses ComfyUI's
        # memory manager -- if VRAM regresses on the 2 MP upscale pass, the
        # patcher-aware path is the upgrade.
        upscale_model = getattr(upscale_model, "model", upscale_model)
...
NODE_CLASS_MAPPINGS = {"RaccoonLatentUpsamplerTiled": LTXVLatentUpsamplerTiled}
NODE_DISPLAY_NAME_MAPPINGS = {"RaccoonLatentUpsamplerTiled": "Raccoon LTX Latent Upsampler (Tiled)"}
