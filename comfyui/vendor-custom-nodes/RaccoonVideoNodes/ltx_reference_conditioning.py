...
VENDORED verbatim from TenStrip/10S-Comfy-nodes @ c412da52 (MIT — see
LICENSE-10S_Nodes in this folder). Registered as
`RaccoonLTXReferenceConditioning` so nothing collides if the pack is ever
installed alongside; the class name is left as-is to keep diffs readable.
Vendored together with latent_upsampler_tiled.py, which is what freed ComfyUI
core from its pin — see that file's header. `LTXReferenceProbe` and
`LTXReferenceBypass` below are upstream's and stay unregistered: the video
workflow never uses them, and deleting them would only make future diffs noisier.
...
NODE_CLASS_MAPPINGS = {"RaccoonLTXReferenceConditioning": LTXReferenceConditioning}
NODE_DISPLAY_NAME_MAPPINGS = {"RaccoonLTXReferenceConditioning": "Raccoon LTX Reference Conditioning"}
