...
import { Wand2, Clapperboard, Images, Film, Package, ScrollText, ExternalLink, SlidersHorizontal, Puzzle, PencilRuler, Settings, Wrench } from 'lucide-react'
...
import { navGroups } from '@/lib/features/registry'
import { useAddonStore } from '@/lib/addons/store'
import { Fragment, useEffect, useState } from 'react'
import QueuePanel from '@/components/queue/QueuePanel'

const ICONS: Record<string, LucideIcon> = {
  Wand2, Clapperboard, Images, SlidersHorizontal, Film, Package, ScrollText, Puzzle, PencilRuler, Wrench,
}
      ...
      <header className="relative h-[4.5rem] grid grid-cols-[1fr_minmax(0,auto)_1fr] items-center gap-3 px-5 border-b border-border bg-card/80 backdrop-blur-xl shrink-0">
        ...
        <div className="flex items-center gap-2.5 shrink-0 justify-self-end">
          ...
          <Link
            ...
            aria-label="Settings"
            title="Settings"
            className="flex items-center justify-center rounded-lg border border-border h-8 w-8 text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
          >
